import { resolve } from 'path';
import Components from 'unplugin-vue-components/vite';
import { defineConfigWithTheme } from 'vitepress';

import { nav, sidebar } from './menu';

import { MarkdownTransform } from '../plugins/MarkdownTransform';

export default defineConfigWithTheme({
  title: 'Redlonely の 小站',
  description: '个人技术文档',
  locales: {
    root: { label: '简体中文', lang: 'zh-Hans' },
  },
  srcDir: 'docs',
  outDir: '../dist',
  lastUpdated: true,
  head: [
    ['meta', { name: 'referrer', content: 'no-referrer-when-downgrade' }],
    [
      'meta',
      {
        name: 'keywords',
        content:
          '前端学习、前端面试、前端算法、编程学习、bilibili、vitepress、随笔、资源导航、React、Vue、Next.js、思维导图、github',
      },
    ],
    ['meta', { name: 'author', content: 'redlonely' }],
    ['meta', { property: 'og:type', content: 'article' }],
    ['meta', { name: 'application-name', content: 'redlonely' }],
    ['meta', { name: 'apple-mobile-web-app-title', content: 'redlonely' }],
    [
      'meta',
      { name: 'apple-mobile-web-app-status-bar-style', content: 'default' },
    ],
    ['link', { rel: 'shortcut icon', href: '/favicon.ico' }],
    ['link', { rel: 'icon', type: 'image/x-icon', href: '/favicon.ico' }],
    [
      'link',
      {
        rel: 'apple-touch-icon',
        sizes: '120x120',
        href: '/apple-touch-icon.png',
      },
    ],
  ],

  markdown: {
    theme: 'one-dark-pro',
  },

  themeConfig: {
    logo: {
      src: '/android-chrome-512x512.png',
      alt: 'redlonely',
    },
    socialLinks: [
      { icon: 'github', link: 'https://github.com/redlonely' },
      {
        icon: {
          svg: `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" width="20" height="20" viewBox="0 0 512 512">
                  <path fill="currentColor" d="M488.6 104.1c16.7 18.1 24.4 39.7 23.3 65.7v202.4c-.4 26.4-9.2 48.1-26.5 65.1c-17.2 17-39.1 25.9-65.5 26.7H92.02c-26.45-.8-48.21-9.8-65.28-27.2C9.682 419.4.767 396.5 0 368.2V169.8c.767-26 9.682-47.6 26.74-65.7C43.81 87.75 65.57 78.77 92.02 78h29.38L96.05 52.19c-5.75-5.73-8.63-13-8.63-21.79c0-8.8 2.88-16.06 8.63-21.797C101.8 2.868 109.1 0 117.9 0s16.1 2.868 21.9 8.603L213.1 78h88l74.5-69.397C381.7 2.868 389.2 0 398 0c8.8 0 16.1 2.868 21.9 8.603c5.7 5.737 8.6 12.997 8.6 21.797c0 8.79-2.9 16.06-8.6 21.79L394.6 78h29.3c26.4.77 48 9.75 64.7 26.1zm-38.8 69.7c-.4-9.6-3.7-17.4-10.7-23.5c-5.2-6.1-14-9.4-22.7-9.8H96.05c-9.59.4-17.45 3.7-23.58 9.8c-6.14 6.1-9.4 13.9-9.78 23.5v194.4c0 9.2 3.26 17 9.78 23.5s14.38 9.8 23.58 9.8H416.4c9.2 0 17-3.3 23.3-9.8c6.3-6.5 9.7-14.3 10.1-23.5V173.8zm-264.3 42.7c6.3 6.3 9.7 14.1 10.1 23.2V273c-.4 9.2-3.7 16.9-9.8 23.2c-6.2 6.3-14 9.5-23.6 9.5c-9.6 0-17.5-3.2-23.6-9.5c-6.1-6.3-9.4-14-9.8-23.2v-33.3c.4-9.1 3.8-16.9 10.1-23.2c6.3-6.3 13.2-9.6 23.3-10c9.2.4 17 3.7 23.3 10zm191.5 0c6.3 6.3 9.7 14.1 10.1 23.2V273c-.4 9.2-3.7 16.9-9.8 23.2c-6.1 6.3-14 9.5-23.6 9.5c-9.6 0-17.4-3.2-23.6-9.5c-7-6.3-9.4-14-9.7-23.2v-33.3c.3-9.1 3.7-16.9 10-23.2c6.3-6.3 14.1-9.6 23.3-10c9.2.4 17 3.7 23.3 10z"></path>
                </svg>`,
        },
        link: 'https://github.com/redlonely',
      },
      {
        icon: {
          svg: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M3.5 3.25a.75.75 0 0 1 .75-.75C14.053 2.5 22 10.447 22 20.25a.75.75 0 0 1-1.5 0C20.5 11.275 13.225 4 4.25 4a.75.75 0 0 1-.75-.75Zm.75 6.25C10.187 9.5 15 14.313 15 20.25a.75.75 0 0 1-1.5 0A9.25 9.25 0 0 0 4.25 11a.75.75 0 0 1 0-1.5ZM3.5 19a2 2 0 1 1 3.999-.001A2 2 0 0 1 3.5 19Z"></path></svg>`,
        },
        link: 'https://github.com/redlonely',
      },
    ],
    search: {
      provider: 'local',
      options: {
        translations: {
          button: {
            buttonText: '搜索文档',
            buttonAriaLabel: '搜索文档',
          },
          modal: {
            footer: {
              selectText: '选择',
              navigateText: '切换',
              closeText: '关闭',
            },
            noResultsText: '无法找到相关结果',
          },
        },
      },
    },
    nav: nav(),
    sidebar: sidebar(),
    // editLink: {
    //   pattern: "https://github.com/vuejs/vitepress/edit/main/docs/:path",
    //   text: "在 GitHub 上编辑此页面",
    // },
    outline: {
      label: '页面导航',
      level: 'deep',
    },
    docFooter: {
      prev: '上一篇',
      next: '下一篇',
    },
    lastUpdatedText: '最后更新于',
    langMenuLabel: '多语言',
    returnToTopLabel: '回到顶部',
    sidebarMenuLabel: '菜单',
    darkModeSwitchLabel: '主题',
    outlineTitle: '导航栏',
    footer: {
      message: '基于 MIT 许可发布',
      copyright: `版权所有 © 2019 - ${new Date().getFullYear()} | Redlonely\`s <p><a href="https://beian.miit.gov.cn/" target="_blank">鄂ICP备20004008号-2</a></p>`,
    },
  },
  vite: {
    plugins: [
      MarkdownTransform(),
      Components({
        dirs: resolve(__dirname, '.vitepress/theme/components'),
        include: [/\.vue$/, /\.vue\?vue/, /\.md$/],
        resolvers: [
          // IconsResolver({
          //   componentPrefix: '',
          // }),
        ],
        dts: './types/components.d.ts',
        transformer: 'vue3',
      }),
    ],
  },
});
